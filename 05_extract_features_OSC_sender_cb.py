...
client = udp_client.SimpleUDPClient('127.0.0.5', 5014) #this client sends to SC
...
def tf_handler(args):
  ...
  data = {"instances": [args]}
  r = requests.post(url = "http://localhost:8541/v1/models/improv_class:predict", data=json.dumps(data), headers=headers)
  response = r.json()
